...
class Report:
    def __init__(self,
        ...
        '''
            ...
            model       : model-file
            directory   : str   path to templates and were to save report
        '''
        self.model = model
    ...
    def calculate_train_test_metrics(self):
        table_train = self.calc_metrics(
                        self.y_train,
                        self.predicted_train,
                        self.task)
        table_test = self.calc_metrics(
                        self.y_test,
                        self.predicted_test,
                        self.task)
    ...
    def model_parameters_to_list(self):
        ...
        for table_name, table in self.get_objects_as_dicts(self.model):
            model_parameters_list.append(self.create_html_table(
                    [table_name],
                    table,
    ...
    def get_objects_as_dicts(self, obj: any, path='') -> list:
        ...
        if obj is None:
            ...
            for item in obj:
                result.extend(self.get_objects_as_dicts(
                    item,
                    path=f"{path}")
                )
