    ...
    for pair in pairs_for_matrix:
        ...
        for gr in unique_gr:
            for gr_2 in unique_gr:
                ...
                subset_ = col0.between(gr.left, gr.right) & col1.between(gr_2.left, gr_2.right)
                df_compare.loc[gr, gr_2] = pair_df.loc[subset_, 'Profit'].sum() / pair_df.loc[subset_, 'Profit'].count()
                df_count.loc[gr, gr_2] = pair_df.loc[subset_, 'Profit'].count()
...
def _cut_column(column, groups_type, bins=None, start=None, end=None, freq=None):
    ...
    try:
        ...
        if groups_type == 'cut':
            ...
            _start = min(column) if start is None else start
