...
class FeatureSelection:
    ...
    Parameters:
        ...
        technique (str): A technique to compute features importance. Values 
    ...
    def __init__(self, y_column, task, method='random_forest', 
    ...
    def create_model(self, df):
        ...
        if (len([var for var in df.columns if df[var].dtype=='object']) > 0):
            raise ValueError(
                'All values in the dataframe must not be object.')
    ...
    def create_permutation_importance(self, scoring=None, n_repeats=5, 
                                      n_jobs=None, sample_weight=None, 
                                      max_samples=1.0):
        """
        ...
        try:
            ...
            raise Exception(
                'Permutation importance can only be used with the estimator.')
    ...
    def create_new_dataset(self, threshold = 'mean'):
        ...
        try: 
            df_scores = pd.DataFrame({'feature_name':self.x.columns, 
                        'feature_score': self.importances})
    ...
    def _init_methods_dict(self):
        ...
        if (self.task == 'class'):
            ...
            f'Value task must be one of the {self.tasks_list}')
