...
=======
    ...
    is `None` it use current working directory as a source. 
    If you want to create new models set the `create_models` parameter to 
    True.
    If you already have source parameter and set `create_models` parameter to 
    True new models will be added to the source list.
    
    Parameters:
        X (:obj:`pd.DataFrame`, :obj:`pd.Series`): Data for making 
            predictions.
        y (:obj:`pd.DataFrame`, :obj:`pd.Series`): Actual target values for X.
        task (:obj:`str`, :obj:`None`): A task for models and metrics. 
            If `task` new models will be created Supports 'reg' and 'class'.
        create_models (:obj:`bool): If True, new models will be created and 
            added to the comparison list.
        source (:obj:`str`, :obj:`list`, :obj:`tuple`, :obj:`None`): List or 
            tuple of insolver wrappers or path to the folder with models. 
            If `None`, taking current working directory as source.
        metrics (:obj:`list`, :obj:`tuple`, :obj:`callable`, optional): 
            Metrics or list of metrics to compute.
        stats (:obj:`list`, :obj:`tuple`, :obj:`callable`, optional): 
            Statistics or list of statistics to compute.
        h2o_init_params (:obj:`dict`, optional): Parameters passed to 
            `h2o.init()`, when `backend` == 'h2o'.
        predict_params (:obj:`list`, optional): List of dictionaries 
...
>>>>>>> Stashed changes
    ...
    def __init__(self, X, y, task=None, create_models=False, 
                 source=None, metrics=[], stats=None, 
    ...
    def compare(self):
        ...
        If `self.create_models` == True, new models will be created and 
        added to the source list.
            
        Raises:
            Exception: `task` parameter must be initialized
                and be `class` or `reg`.
    ...
    def _init_new_models(self):
        ...
        If `class` then Gradient Boosting model with the catboost
        backend and Random Forest with the sklearn backend will be 
        created.
        If `reg` then Gradient Boosting model with the catboost
        backend, Random Forest with the sklearn backend and Linear Model
        with the sklearn backend will be created.
        This method uses train_test_split from sklearn.model_selection,
        fits models with train values and changes `self.X`, `self.y`
        to test values. Thus, when calculating metrics, it will use 
        test values.
        ...
        if (self.task == 'class'):
            models_dict = {
                'new_gbm': InsolverGBMWrapper(backend='catboost', 
                                              task='class',
                                              n_estimators=10), 
                'new_rf': InsolverRFWrapper(backend='sklearn', 
                                            task='class'),
            }
        elif (self.task == 'reg'):
            models_dict = {
                'new_glm': InsolverGLMWrapper(backend='sklearn', family=0, 
                                              standardize=True),
                'new_gbm': InsolverGBMWrapper(backend='catboost', 
                                              task='reg',
                                              n_estimators=10), 
    ...
    def _init_source_models(self):
        ...
        Raises:
            Exception: Models with the insolver name format were not found
        ...
        if (self.source is None) or isinstance(self.source, str):
            ...
            files = [file for file in files 
                     if os.path.basename(file).split('_')[0] == 'insolver']
            if files:
                model_list = []
                for file in files:
                    algo, backend = os.path.basename(file).split('_')[1:3]
                    model_list.append(wrappers[algo](backend=backend,
                                                     load_path=file) 
                                      if backend != 'h2o' else
                                      wrappers[algo](backend=backend, 
                                                     load_path=file, 
                                                     h2o_init_params=
                                                     self.h2o_init_params))
    ...
    def _calc_metrics(self):
        ...
        Returns:
            Returns `None`, but results available in `self.stats`, 
            `self.metrics`.
        ...
        features = ([None] + self.features if self.features is not None 
                    else None)
        for model in models:
            algos.append(
                model.algo.upper()) if hasattr(model, 
                                               'algo') else algos.append('-')
            (backend.append(
                model.backend.capitalize()) if hasattr(model, 'backend') 
                                            else backend.append(
                                                model.__class__.__name__))
                
            p = model.predict(self.X if (features is None) 
                              or (features[models.index(model)] is None)
                              else self.X[features[models.index(model)]], 
                              **({} if (self.predict_params is None) 
                                 or (self.predict_params[models.index(model)] 
                                     is None)
                                 else self.predict_params[models.index(model)]
                                ))
            
            stats_val = [mean(p), var(p), std(p), min(p), quantile(p, 0.25), 
                        median(p), quantile(p, 0.75), max(p)]
                
            name_stats = ['Mean', 'Variance', 'St. Dev.', 'Min', 'Q1', 
                          'Median', 'Q3', 'Max']
                ...
                if isinstance(self.stats, (list, tuple)):
                    for stat in self.stats:
                        if callable(stat):
                            ...
                            raise TypeError(
                                f'Statistics with type {type(stat)} are not supported.')
                    ...
                    raise TypeError(
                        f'Statistics with type {type(self.stats)} are not supported.')
                    
            stats_df = stats_df.append(DataFrame([stats_val], 
                                                 columns=name_stats))
                ...
                if isinstance(self.metrics, (list, tuple)):
                    ...
                    for metric in self.metrics:
                        if callable(metric):
                            ...
                            raise TypeError(
                                f'Metrics with type {type(metric)} are not supported.')
                            
                    metrics_names = [m.__name__.replace('_', ' ') 
                                     for m in self.metrics]
                    model_metrics = model_metrics.append(
                        DataFrame(dict(zip(metrics_names, m_metrics), 
                                       index=[0])))
        ...
        stats_df = stats_df[list(stats_df.columns[-2:]) 
                            + list(stats_df.columns[:-2])]
        model_metrics = model_metrics[list(model_metrics.columns[-2:]) 
                                      + list(model_metrics.columns[:-2])]
