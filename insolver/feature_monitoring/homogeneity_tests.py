...
from psi_homogeneity_test import (
    psi_discr_2samp,
    psi_cont_2samp,
    sec_min
)
...
class DiscreteHomogeneityTests:
    ...
    def __fillna(self, x1, x2):
        ...
        if x1.dtype == 'int' or x1.dtype == 'float':
            nan_value = min(min(x1[~np.isnan(x1)]),
                            min(x2[~np.isnan(x2)])) - 1
    ...
    def run_all(self, x1_ref, x2_ref):
        ...
        if (x1_ref.shape[0] < self.samp_size * 1.7) or (x2_ref.shape[0] < self.samp_size * 1.7):
            raise Warning("Sizes of x1 and x2 are better to be several times greater " +
