...
class NoNameFS:
    ...
    Parameters:
        ...
        fillna (bool, None), default=True: Auto fill NA bool: if True Auto fill NA will be applied, if False/None it won't 
         be applied.
        ...
        feature_selection (str, bool, None), default=None: Feature selection method name. If True `random_forest` will be 
         used. If False/None feature selection won't be applied.
        feat_select_task (str), default=None: Feature selection task, values `reg`, `class`, `multiclass`, 
         `multiclass_multioutput` are supported.
        feat_select_threshold (str, int), default='mean': Feature selection threshold, values `mean`, `median` are supported
         or the threshold can be numeric.
        dim_red (str, bool, None), default=None: Dimensionality reduction method name, values `pca`, `svd`, `lda`, `t_sne`, 
         `isomap`, `lle`, `fa`, `nmf` are supported. If True `pca` will be used. If False/None dimensionality reduction
         won't be applied.
        ...
        dim_red_n_neighbors (int, None), default=None: Dimensionality reduction n_neighbors (or perplexity in the `t_sne`) 
        ...
        smoothing (str, bool, None), default=None: Smoothing method name, values `moving_average`, `lowess`, `s_g_filter`, 
         `fft` are supported. If True `moving_average` will be used. If False/None smoothing won't be applied.
    ...
    def no_name_func(self, df, target=None, drop_target=True):
        ...
        transformes categorical columns using the `one_hot_encoder` method, fills in the NA values using the 'median' method 
        for numerical columns and the 'frequent' method for categorical columns, normalizes data using the `standard` method.
